<x-app-layout>
    ...
    <div class="py-6 max-w-7xl mx-auto" class_old="py-6 max-w-4xl mx-auto">
        ...
        <form method="POST" action="{{ route('therapies.update', $therapy) }}" class="bg-white p-6 rounded shadow space-y-4">
            @csrf
            @if(isset($therapy))
                @method('PUT')
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <x-text-input name="ptid" class="w-full" :value="$therapy->ptid" />
                ...
                <div>
                    ...
                    <select name="therapycategoryid"
                        ...
                        @foreach($therapycategories as $id => $therapycatname)
                            <option value="{{ $id }}" @selected( ($therapy->therapycategoryid) == $id)>
                                {{ $therapycatname }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <div class="border rounded-md p-3 space-y-2">
                    ...
                    <div>
                        <x-input-label class="text-sm" for="therapyname[hu]" value='HU' />
                        <x-text-input  type="text" name="therapyname[hu]"
                               class="w-full border rounded px-2 py-1"
                               :value='$therapy->therapyname["hu"] ?? "..."' />
                    </div>

                    <div>
                        <label class="text-sm">EN</label>
                        <x-text-input type="text" name="therapyname[en]"
                               class="w-full border rounded px-2 py-1"
                               :value='$therapy->therapyname["en"]' />
                    </div>

                    <div>
                        <label class="text-sm">DE</label>
                        <x-text-input type="text" name="therapyname[de]"
                               class="w-full border rounded px-2 py-1"
                               :value='$therapy->therapyname["de"]' />
                    </div>
            ...
            <div>
                ...
                <select name="hdprimarycategoryid"
                    ...
                    @foreach($primaryCategories as $id => $pricatname)
                        <option value="{{ $id }}" @selected( ($therapy->hdprimarycategoryid) == $id)>
                            {{ $pricatname }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="hdsecondarycategoryid"
                    ...
                    @foreach($secondarycategories as $id => $seccatname)
                        <option value="{{ $id }}" @selected(  ($therapy->hdsecondarycategoryid) == $id)>
                            {{ $seccatname }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <div class="flex gap-6 mt-1">
                    <label class="flex items-center gap-4">
                        <input type="checkbox" name="lowpressure" value="1" {{  $therapy->lowpressure ? 'checked' : '' }}>
                        {{__('bd.low')}}
                    </label>

                    <label class="flex items-center gap-4">
                        <input type="checkbox" name="midpressure" value="1" {{  $therapy->midpressure ? 'checked' : '' }}>
                        {{__('bd.middle')}}
                    </label>

                    <label class="flex items-center gap-4">
                        <input type="checkbox" name="highpressure" value="1" {{ $therapy->highpressure ? 'checked' : '' }}>
                        {{__('bd.high')}}
                    </label>                
                </div>
            </div>
            ...
            <div>
                ...
                <x-text-input type="number" name="temperature" class="w-full" :value="$therapy->temperature" />
            </div>
            ...
            <div>
                ...
                <div class="flex gap-6 mt-1">
                    ...
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="timerangeshort" value="1" {{ $therapy->timerangeshort ? 'checked' : '' }}>
                        {{__('bd.short')}}
                    </label>

                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="timerangemiddle" value="1" {{ $therapy->timerangemiddle ? 'checked' : '' }}>
                        {{__('bd.middle')}}
                    </label>

                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="timerangelong" value="1" {{ $therapy->timerangelong ? 'checked' : '' }}>
                        {{__('bd.long')}}
                    </label>
                ...
                <div class="border rounded-md p-3 space-y-2">
                    ...
                    <div>
                        ...
                        <textarea name="therapyinfo[hu]" rows="3"
                            class="w-full border rounded px-2 py-1">{{ $therapy->therapyinfo["hu"] }}</textarea>
                    </div>

                    <div>
                        <label class="text-sm">EN</label>
                        <textarea name="therapyinfo[en]" rows="3"
                            class="w-full border rounded px-2 py-1">{{ $therapy->therapyinfo["en"] }}</textarea>
                    </div>

                    <div>
                        <label class="text-sm">DE</label>
                        <textarea name="therapyinfo[de]" rows="3"
                            class="w-full border rounded px-2 py-1">{{ $therapy->therapyinfo["de"] }}</textarea>
                    </div>
