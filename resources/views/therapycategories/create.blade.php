    ...
    <div class="py-6 max-w-xl mx-auto">
        <form method="POST" action="{{ isset($therapycategory) ? route('therapycategories.update', $therapycategory) : route('therapycategories.store') }}" class="bg-white p-6 rounded shadow">
            ...
            <div class="mb-4">
                ...
                <x-text-input name="therapycatname[hu]" :value="$therapycategory->therapycatname['hu'] ?? ''" />
            </div>
            
            <div class="mb-4">
                <x-input-label for="therapycatname[en]" value="English" />
                <x-text-input name="therapycatname[en]" :value="$therapycategory->therapycatname['en'] ?? ''" />
            </div>

            <div class="mb-4">
                <x-input-label for="therapycatname[de]" value="Deutsch" />
                <x-text-input name="therapycatname[de]" :value="$therapycategory->therapycatname['de'] ?? ''" />
            </div>
